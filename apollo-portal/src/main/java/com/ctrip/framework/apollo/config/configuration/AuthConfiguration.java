...
import com.ctrip.framework.apollo.config.defaultimpl.DefaultLogoutHandler;
import com.ctrip.framework.apollo.config.defaultimpl.DefaultSsoHeartbeatHandler;
import com.ctrip.framework.apollo.config.springsecurity.SpringSecurityUserInfoHolder;
import com.ctrip.framework.apollo.config.springsecurity.SpringSecurityUserService;
import com.ctrip.framework.apollo.filter.JWTAuthenticationFilter;
import com.ctrip.framework.apollo.filter.JWTLoginFilter;
...
//                            "select Username,Authority from `Authorities` where Username = ?")
                    .authoritiesByUsernameQuery(
                            "select Username,Password as Authority from `Users` where Username = ?")
    ...
    static class SpringSecurityConfigurer extends WebSecurityConfigurerAdapter {
        ...
        public static final String ADMIN_ROLE = "apollo";
        ...
        @Bean
        YyCorsFilter YyCorsFilter() {
            YyCorsFilter filter = new YyCorsFilter();
            return filter;
        }
        ...
        protected void configure(HttpSecurity http) throws Exception {
            ...
            http.authorizeRequests()
                    ...
                    .antMatchers("/AA").hasAnyRole(USER_ROLE,ADMIN_ROLE).anyRequest().authenticated().and().cors().
                    and().formLogin().loginPage("/login").
                    permitAll().successHandler(myAuthenticationSuccessHandler).
                    failureHandler(myAuthenctiationFailureHandler);
            http.addFilterBefore(YyCorsFilter(), HeaderWriterFilter.class).addFilter(new JWTLoginFilter(authenticationManager())).addFilter(new JWTAuthenticationFilter(authenticationManager()));
